...
if [[ $ubuntu_os_type =~ "Ubuntu" ]]; then
    sudo apt update
    sudo apt install -y build-essential curl zsh vim-gtk3 bat universal-ctags ripgrep silversearcher-ag tmux xsel ranger shellcheck safe-rm
    ...
    cp .tmux/plugins/tmux-agent-indicator/plugins/opencode-tmux-agent-indicator.js ~/.config/opencode/plugins/opencode-tmux-agent-indicator.js
...
#4 curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.7/install.sh | bash && nvm install --lts && npm install -g typescript@5
#5 git clone git@github.com:pop-os/shell.git .popos_shell -> use popshell
#6 docker build use network-proxy: .http-proxy.conf -> /etc/systemd/system/docker.service.d/http-proxy.conf
